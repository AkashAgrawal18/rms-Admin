...
class Login_model extends CI_model{
	...
	$this->db->select('m_admin_id,m_admin_type,m_admin_name');

	$this->db->where('m_admin_email',$this->input->post('login_email'));

	$this->db->where('m_admin_pass',$pass);
	$this->db->where('is_login_allow',1);
  ...
  public function countRow_dashboard()
	{
		...
		$customer = $this->db->where('m_acc_type',3)->get('master_accounts_tbl')->num_rows();
		$inactive_customer = $this->db->where('m_acc_type',3)->where('m_acc_status',2)->get('master_accounts_tbl')->num_rows();
    ...
    $poffer = $this->db->get('product_offers')->num_rows();
     // $totalsales = $this->db->get('product_offers')->num_rows();
		...
		return $data ;
				    ...
				    $this->db->select('customer.m_acc_name AS customer_name,customer.m_acc_id as customer_id,customer.m_acc_mobile as customer_mobile');
